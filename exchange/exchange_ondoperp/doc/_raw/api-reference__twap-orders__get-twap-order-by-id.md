...
> Returns a single TWAP order by its ID. For how TWAP execution works, see [TWAP orders](/time-weighted-average-price).



## OpenAPI

````yaml /api-reference/rest-spec.json get /v1/perps/twap/order/{orderID}
openapi: 3.0.3
info:
...
paths:
  /v1/perps/twap/order/{orderID}:
    get:
      ...
      description: >-
        Returns a single TWAP order by its ID. For how TWAP execution works, see
