...
````yaml /api-reference/rest-spec.json post /v1/perps/orders
openapi: 3.0.3
info:
...
paths:
  /v1/perps/orders:
    post:
      ...
      description: >-
        Creates a limit or market order. Change the "type" field to switch
        between order types. If `postOnly` is `true` and an order would match at
        placement, a 400 is returned with error `post_only_has_match`. For the
        available order types and their behavior, see [Order
        types](/order-types).
