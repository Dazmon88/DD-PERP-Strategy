...
> Cancels an individual order by order ID. The `orderID` path parameter can be:
...
````yaml /api-reference/rest-spec.json delete /v1/perps/orders/{orderID}
openapi: 3.0.3
info:
...
paths:
  /v1/perps/orders/{orderID}:
    delete:
      ...
      description: >
        Cancels an individual order by order ID. The `orderID` path parameter
        can be:
        ...
        Returns a 400 with error code `order_already_cancelled`,
        `order_not_in_cancellable_state`, or `order_already_filled` if the order
        cannot be cancelled.
