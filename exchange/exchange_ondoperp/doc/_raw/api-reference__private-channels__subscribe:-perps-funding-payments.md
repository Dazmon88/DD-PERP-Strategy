...
> Subscribe to the `fundingPaymentsPerps` channel. Requires authentication (login first).



## OpenAPI

````yaml /api-reference/ws-spec.json post /ws/fundingPaymentsPerps
openapi: 3.0.3
info:
...
paths:
  /ws/fundingPaymentsPerps:
    post:
      ...
      description: >-
        Subscribe to the `fundingPaymentsPerps` channel. Requires authentication
        (login first).
