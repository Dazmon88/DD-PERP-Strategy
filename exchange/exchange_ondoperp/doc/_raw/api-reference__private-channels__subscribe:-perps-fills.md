...
> Subscribe to the `fillsPerps` channel. Requires authentication (login first).
...
````yaml /api-reference/ws-spec.json post /ws/fillsPerps
openapi: 3.0.3
info:
...
paths:
  /ws/fillsPerps:
    post:
      ...
      description: >-
        Subscribe to the `fillsPerps` channel. Requires authentication (login
        first).
